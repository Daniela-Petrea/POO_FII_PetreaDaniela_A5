...
void Circuit::Race()
{
		...
		{
			double time = ((1.0) * lungime) / (car[i]->getspeed());
			car[i]->setracetime(time);
			double consumare = ((1.0) * car[i]->getfuelconsumption() * lungime) / 100;
			car[i]->setraceconsumption(consumare);
			if (consumare < car[i]->getfuelcapacity())
				car[i]->terminare = 1;
			else
				car[i]->terminare = 0;
		}
...
void Circuit::ShowFinalRanks() {
	...
	{
		for (int j = i + 1; j < this->maxIndex; j++)
		{
